...
class FollowTraj(Node):
    ...
    def __init__(self):
        ...
        blueberry_position = np.array([0.1, 0.1, 0.1])
        blueberry_joint_state, _ = self.robot.inverse_kinematics_newton_raphson(blueberry_position)
        ...
        if move_j:
            self.move_j(blueberry_joint_state, 10, 100)
        if move_l:
            self.move_l(blueberry_position, 10, 100)
        #Step 2: Manually close the gripper to not crush the blueberry
        self.gripper_control()

        #Step 3: Move the blueberry right above the cup
        dropping_position = np.array([-0.1, -0.1, 0.15])
        dropping_joint_state, _ = self.robot.inverse_kinematics_newton_raphson(dropping_position)
        if move_j:
            self.move_j(dropping_joint_state, 10, 100)
        if move_l:
            self.move_l(dropping_position, 10, 100)

        #Step 4: Manually open the gripper to drop the blueberry in the cup
        self.gripper_control()

        #Step 5: Move back to home position 
        self.go_home(self.current_joints)

        #Step 6: Now to back to the position of the blueberry and pick it back up
        self.move_j(dropping_joint_state, 10, 100)

        #Step 7: Go back home as a midpoint
        self.go_home(self.current_joints)

        #Step 7: Now put it back in the original position
        self.move_j(blueberry_joint_state)
